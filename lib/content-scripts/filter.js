chrome.storage.sync.get(['stupids', 'anonymity-filter'], (items) => {
    ...
    // at the first load
    var answers = document.getElementsByClassName("AnswerItem");
    ...
    function modify_answer_num() {
        ...
        if (stupid_answer_number !== 0)
            stupid_filter_result = stupid_answer_number + " 个黑名单用户被阻挡";
    ...
    function filter_answer(ele) {
        var who = eval('(' + ele.dataset.zop + ')')["authorName"];
